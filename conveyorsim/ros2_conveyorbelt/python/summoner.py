...
class InteractiveSpawner:
    def __init__(self):
        ...
        self.package = "conveyor_description"
        # Define available objects and their default heights
        self.available_objects = {
            '1': {'name': 'redcube', 'default_z': 2.0},
            '2': {'name': 'greencube', 'default_z': 2.0},
            '3': {'name': 'bluecube', 'default_z': 2.0},
            '4': {'name': 'aruco1', 'default_z': 2.0},           
            '5': {'name': 'aruco2', 'default_z': 2.0},
            '6': {'name': 'aruco3', 'default_z': 2.0}                        
        }
    ...
    def get_position_input(self) -> Dict[str, float]:
        ...
        while True:
            try:
                x = float(input("Enter X position (default -0.2): ") or "-0.2")
                y = float(input("Enter Y position (default 0): ") or "0")
                z = float(input("Enter Z position (default 1.0): ") or "1.0")
                return {'x': x, 'y': y, 'z': z}
            except ValueError:
